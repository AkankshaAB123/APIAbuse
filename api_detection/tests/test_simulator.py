...
class SimulatorScenarioTests(TestCase):
    ...
    def test_network_brute_force_scenario_detects_with_failed_history(self):
        ...
        self.assertTrue(result.detected)
        self.assertEqual(result.attack_type, AttackType.NETWORK_BRUTE_FORCE)
        self.assertEqual(result.domain, DetectorDomain.NETWORK)
    ...
    def test_suspicious_process_scenario_detects(self):
        ...
        self.assertTrue(result.detected)
        self.assertEqual(result.attack_type, AttackType.SUSPICIOUS_PROCESS_EXECUTION)
        self.assertEqual(result.domain, DetectorDomain.ENDPOINT)
    ...
    def test_endpoint_privilege_escalation_scenario_detects(self):
        ...
        self.assertTrue(result.detected)
        self.assertEqual(result.attack_type, AttackType.PRIVILEGE_ESCALATION)
        self.assertEqual(result.domain, DetectorDomain.ENDPOINT)

    def test_fake_shopping_event_construction(self):
        event = fake_shopping_event()
        # Verify key fields in request.body
        body = event.request.body
        required_keys = {"url", "domain", "brand", "product_id", "price", "payment_method", "amount", "transaction_id", "scenario", "context"}
        for key in required_keys:
            self.assertIn(key, body)
        self.assertEqual(event.request.method, "POST")
        self.assertEqual(event.request.endpoint, "/api/checkout")
        self.assertEqual(event.domain, DetectorDomain.API)

    def test_fake_bank_event_construction(self):
        event = fake_bank_event()
        body = event.request.body
        required_keys = {"url", "suspicious_url", "domain", "brand", "login_prompt", "verification_url", "scenario", "context"}
        for key in required_keys:
            self.assertIn(key, body)
        self.assertEqual(event.request.method, "GET")
        self.assertEqual(event.request.endpoint, "/api/phishing/link")
        self.assertEqual(event.domain, DetectorDomain.API)
